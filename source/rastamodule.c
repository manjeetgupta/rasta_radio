...
//   allocateRastaByteArray(&result,packet.length);
    allocateRastaByteArray_pool(&result,packet.length,9); // MAY_19
    //check packet length
...
struct RastaByteArray rastaModuleToBytes(struct RastaPacket packet, rasta_hashing_context_t * hashing_context) {
    ...
    allocateRastaByteArray_pool(&data_to_hash, (unsigned int)(packet.length - checksum_len),10);
    ...
    freeRastaByteArray_pool(&data_to_hash);
...
struct RastaPacket bytesToRastaPacket(struct RastaByteArray data, rasta_hashing_context_t * hashing_context) {
    ...
    allocateRastaByteArray_pool(&result.data,len,11); //MAY_19
    ...
    allocateRastaByteArray_pool(&data_to_hash, (unsigned int)(result.length - checksum_len),12);
...
//    allocateRastaByteArray(&result.checksum, checksum_len);
    allocateRastaByteArray_pool(&result.checksum, checksum_len,13);  //MAY_19
...
struct RastaByteArray rastaRedundancyPacketToBytes(struct RastaRedundancyPacket packet, rasta_hashing_context_t * hashing_context){
    ...
    struct RastaByteArray result;
//    allocateRastaByteArray(&result, packet.length);
    allocateRastaByteArray_pool(&result, packet.length,14);   //MAY_23
...
//    freeRastaByteArray(&internal_packet_bytes);
    freeRastaByteArray_pool(&internal_packet_bytes);   //MAY_19
    ...
    allocateRastaByteArray_pool(&temp_wo_checksum, len_wo_checksum,15);
    // copy data
    ...
    freeRastaByteArray_pool(&temp_wo_checksum);
...
struct RastaRedundancyPacket bytesToRastaRedundancyPacket(struct RastaByteArray data, struct crc_options checksum_type, rasta_hashing_context_t * hashing_context){
    ...
    allocateRastaByteArray_pool(&internal_packet_bytes, data_len,16);
 ...
 *  for(int j=0;j<data_len;++j)
    {
        PRINT_LINE("%02X ",data.bytes[8+j]);
    }
    PRINT_LINE("\r\n");

*/

   // memcpy(internal_packet_bytes.bytes,data.bytes+8,data_len);
    ...
    //freeRastaByteArray(&internal_packet_bytes);
    freeRastaByteArray_pool(&internal_packet_bytes);
    ...
    allocateRastaByteArray_pool(&data_wo_checksum, data_wo_checksum_len,17);
    // copy data
    ...
    freeRastaByteArray_pool(&data_wo_checksum);
    // checksum check
