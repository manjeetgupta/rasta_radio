...
void config_setstd(struct RastaConfig * cfg) {
    ...
    if (entr.type == DICTIONARY_NUMBER){
        ...
        cfg->values.sending.sr_hash_key = 0x12345678;
    }
...
void config_setstd_baremetal(struct RastaConfig * cfg) {
    struct DictionaryEntry entr;
    ...
    cfg->values.sending.md4_a = 0x67452301;                         //RASTA_MD4_A
    cfg->values.sending.md4_b = 0xefcdab89;                         //RASTA_MD4_B
    cfg->values.sending.md4_c = 0x98badcfe;                         //RASTA_MD4_C
    cfg->values.sending.md4_d = 0x10325476;                         //RASTA_MD4_D
    cfg->values.sending.send_max = 20;                              //RASTA_SEND_MAX
    cfg->values.sending.mwa = 10;                                   //RASTA_MWA
    cfg->values.sending.max_packet = 1;                             //RASTA_MAX_PACKET
    cfg->values.sending.diag_window = 5000;                         //RASTA_DIAG_WINDOW


    // Redundancy part

    cfg->values.redundancy.connections.count = 0;                   //RASTA_REDUNDANCY_CONNECTIONS
    cfg->values.redundancy.crc_type = crc_init_opt_a();             //RASTA_CRC_TYPE
    cfg->values.redundancy.t_seq = 100;                             //RASTA_T_SEQ
    cfg->values.redundancy.n_diagnose = 200;                        //RASTA_N_DIAGNOSE
    cfg->values.redundancy.n_deferqueue_size = 8;                   //RASTA_N_DEFERQUEUE_SIZE

    // General

    cfg->values.general.rasta_network = 0;                          //RASTA_NETWORK
    cfg->values.general.rasta_id = 0;                               //RASTA_ID
...
#ifdef BAREMETAL
struct RastaConfig config_load_baremetal(const char filename[256],struct RastaIPData *static_ipdata)
{
    struct RastaConfig config;
    strcpy(config.filename,"none");
    char rasta_arg[256];


    strcpy(rasta_arg,filename); //Copy the passed argument
    ...
    //initialize standard value
    //config_setstd(&config);
    config_setstd_baremetal(&config);

    config.values.general.rasta_network 		= (unsigned long)1234; // RASTA_ID
    config.values.redundancy.connections.count  = RASTA_MAX_CONNECTIONS; //Number of Port+IP configured
//    config.values.redundancy.connections.data = rmalloc(sizeof(struct RastaIPData) * 2);
    config.values.redundancy.connections.data   = static_ipdata;

    if (strcmp(rasta_arg, "s1") == 0)
    {
        config.values.general.initial_seq_num   = (unsigned long)100; // Initial_seq_number
    	config.values.general.rasta_id 			= (unsigned long)98;//Client=0x62(98),Server=0x61(97)
    	//struct RastaIPData ip[2] = {{"127.0.0.1",9998},{"127.0.0.1",9999}}; //Client
    	struct RastaIPData ip[2] = {{"192.168.25.10",15003},{"192.168.25.10",15004}}; //Client
    	for (unsigned int i = 0; i < 2; i++)
		{
			config.values.redundancy.connections.data[i] = ip[i];
		}
    }
    else if (strcmp(rasta_arg, "s2") == 0)
    {
        config.values.general.initial_seq_num   = (unsigned long)200; // Initial_seq_number
        config.values.general.rasta_id          = (unsigned long)99;//Client=0x63(99),Server=0x61(97)
        //struct RastaIPData ip[2] = {{"127.0.0.1",9998},{"127.0.0.1",9999}}; //Client
        struct RastaIPData ip[2] = {{"192.168.25.82",15005},{"192.168.25.82",15006}}; //Client
        for (unsigned int i = 0; i < 2; i++)
        {
            config.values.redundancy.connections.data[i] = ip[i];
        }
    }
    else if(strcmp(rasta_arg, "r") == 0)
    {
        config.values.general.initial_seq_num   = (unsigned long)500; // Initial_seq_number
        config.values.general.rasta_id 			= (unsigned long)97;//Client=0x62(98),Server=0x61(97)
        //struct RastaIPData ip[2] = {{"127.0.0.1",8888},{"127.0.0.1",8889}}; //Server
        struct RastaIPData ip[2] = {{"192.168.25.83",15000},{"192.168.25.83",15001}}; //Server
        for (unsigned int i = 0; i < 2; i++)
		{
			config.values.redundancy.connections.data[i] = ip[i];
		}
