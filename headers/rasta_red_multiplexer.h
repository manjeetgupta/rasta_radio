...
typedef struct redundancy_mux redundancy_mux;
	...
	struct receive_event_data {
		...
		int channel_index;
		unsigned char receive_buffer[MAX_DEFER_QUEUE_MSG_SIZE];
		unsigned int len; //receive_buffer length
		unsigned int sender_port; //sender port
...
typedef struct
{
    on_diagnostics_available_red_ptr on_diagnostics_available; //called when diagnose information of a transport channel is available
    on_new_connection_ptr  on_new_connection; 				   //called when a new redundancy channel has sent data to this entity.
...
{
    uint16_t * listen_ports; // the ports where this entity will listen (where the udp sockets are bound)


#ifdef BAREMETAL
    struct udp_pcb **udp_socket_fds;	 // the file descriptors of the used udp sockets. array has length port_count
#else
    int * udp_socket_fds;	 // the file descriptors of the used udp sockets. array has length port_count //BAREMETAL_FUNC
#endif

    unsigned int port_count;

    rasta_redundancy_channel * connected_channels; // the redundancy channels to remote entities this multiplexer is aware of
    unsigned int channel_count; // the amount of known redundancy channels

    int is_open; //    1 if the redundancy multiplexer is active, 0 otherwise
    struct logger_t logger;
    struct RastaConfigInfo config;

    rasta_redundancy_notifications notifications; // the notifications of this multiplexer and it's redundancy channels
    unsigned int notifications_running;// amount of notification thread that are currently running
 ...
 * @return an initialized redundancy layer multiplexer
 */
//redundancy_mux redundancy_mux_init_baremetal(struct logger_t logger, struct RastaConfigInfo config,struct rasta_handle *h,struct app_context *ctx);//BAREMETAL_FUNC
