 ...
 */
#define MAX_DEFER_QUEUE_MSG_SIZE 500
...
typedef struct
{
    unsigned long associated_id; 					//* the RaSTA ID of the remote entity this channel is bound to
    rasta_redundancy_state current_state; 			//* current state of the redundancy channel
    struct crc_options checksum_type;				//* the type of checksum that is used for all messages
    unsigned long seq_tx; 							//* next sequence number to send
    unsigned long seq_rx; 							//* next expected sequence number to be received
    struct defer_queue *defer_q; 					//* the defer queue
    struct defer_queue *diagnostics_packet_buffer; 	//* used to store all received packets within a diagnose window
    fifo_t * fifo_recv; 							//* the FIFO where the messages for the upper layer are stored
    struct logger_t logger; 						//* logger used for logging
    int is_open; 									//* 1 if the redundancy channel is open, 0 if it is closed
    struct RastaConfigInfoRedundancy configuration_parameters;

    rasta_transport_channel * connected_channels; 	// * the transport channels of the partner (client) when running in server mode.
    												// * these are dynamically added when a message from the corresponding channel is received.
    unsigned int connected_channel_count; 			//* the amount of discovered partner (client) transport channels
